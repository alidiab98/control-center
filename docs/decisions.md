...
| `TimelineEvent.automation` + `getAutomationsToday()`                                     | Home lists "automations today" across tasks (SPEC 8.2); a per-task `getTimeline` cannot answer that, and a flag on the event is what tells a step you took apart from one the system took. Approved contract change.         |
| Only three fixture events carry `automation: true`                                       | So the home list shows exactly the three lines the screenshot draws. The mock also flags what the system raises at runtime: an agent starting, a Tracker transition, the simulator entry. A person's actions stay unflagged. |
| `getAutomationsToday()` returns newest first, and live automations are prepended         | The home list reads as "what just happened". The mockup's own order is neither chronological nor reverse, so it was treated as arbitrary.                                                                                    |
| `TimelineEvent.taskId` is nullable                                                       | Some automations belong to no task, such as the standup draft. `getTimeline(taskId)` never returns them and the per-task timeline cache ignores them; `getAutomationsToday()` includes them. Approved contract change.       |
| "Today" in `getAutomationsToday()` follows the local calendar day                        | A session left open past midnight shows an empty list until new automations arrive. Accepted: the method name promises today, not the last 24 hours.                                                                         |
| `CC_PORT` (default 5173) drives the Vite port and Playwright's `baseURL` and `webServer` | Three milestones run in parallel worktrees, each needing its own dev server and its own e2e run.                                                                                                                             |
| `strictPort` on, and Playwright never reuses a server when `CC_PORT` is set              | A silent move to the next free port would let one worktree test another worktree's build. A collision must fail loudly instead.                                                                                              |
| Node types are enabled for `web` and `process` is banned in `web/src/**` by lint         | `vite.config.ts` reads `CC_PORT`, which needs Node types; the lint rule keeps them out of browser code. Cheaper than a second tsconfig with project references, which would force `composite` and emit.                      |
